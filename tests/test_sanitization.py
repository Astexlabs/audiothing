...
class TestSanitizeText(unittest.TestCase):
    ...
    def test_backticks_removed(self):
        ...
        self.assertEqual(sanitize_text("Use `python main.py` to run"), "Use python main.py to run")

    def test_single_quotes_and_contractions(self):
        # Contractions and possessives preserved
        self.assertEqual(sanitize_text("don't it's we'll let's O'Connor"), "don't it's we'll let's O'Connor")
        self.assertEqual(sanitize_text("don’t it’s we’ll let’s"), "don't it's we'll let's")
    ...
    def test_hyphens_dashes_and_ranges(self):
        ...
        self.assertEqual(sanitize_text("non-trivial and anti-virus"), "non trivial and anti virus")
    ...
    def test_underscores_removed(self):
        ...
        self.assertEqual(sanitize_text("word_with_multiple___underscores"), "word with multiple underscores")

    def test_markdown_and_formatting(self):
        self.assertEqual(sanitize_text("# Heading\nBody text"), "Heading\nBody text")
        self.assertEqual(sanitize_text("This *may* be **bold** and ***italic***"), "This may be bold and italic")
        self.assertEqual(sanitize_text("1) First item\n2) Second item"), "1. First item\n2. Second item")
        self.assertEqual(sanitize_text("- Bullet one\n- Bullet two"), "Bullet one\nBullet two")
        self.assertEqual(sanitize_text("hardware/software/capabilities"), "hardware, software, capabilities")
        self.assertEqual(sanitize_text("model (replaced by rogue model) done."), "model, replaced by rogue model, done.")
